...
BOOL CreateGLWindow(char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	wchar_t Ltitle[500];
	MultiByteToWideChar(0, 0, title, strlen(title), Ltitle, strlen(title));
	// Create The Window
...
int main()
{
	...
	try {											// Use exception handling 
		...
		wchar_t error[500];
		MultiByteToWideChar(0, 0, e.what(), strlen(e.what()), error, strlen(e.what()));
		MessageBox(NULL, error, L"CAUGHT AN EXCEPTION", MB_OK | MB_ICONINFORMATION);
	}
