...
vec4 GetLightedColor(vec3 loc, vec3 pos, float density){
	...
	vec4 rstColour = vec4(density, 1-density,0,1);
...
void main() {
	...
	//vec2 coord = vec2((gl_FragCoord.x+0.5)/windowWidth, (gl_FragCoord.y+0.5)/windowHeight);
	vec3 start = rawPos;
	...
	for(i = 0;i<sampeRate; i++){
		...
		if(density > threshold){
			...
			vec3 color = GetLightedColor(loc2, pos, density).xyz;
			//vec3 color = vec3(density, 0, 0);
			//accColor = color;
			//accTransparency = 0;
			//break;
