...
class Array3D
	...
	const ElementType & operator() (long x, long y, long z) const								//	use function call to access array elements
	{
#ifdef RANGE_CHECK																	//	if range-checking is turned on
		if (block == NULL)															//	i.e. not constructed yet
		{
	...
	long XDim() const { return xDim; }														//	accessor to find out size
	long YDim() const { return yDim; }														//	accessor to find out size
	long ZDim() const { return zDim; }														//	accessor to find out size
	long RowSize() const { return rowSize; }													//	accessor to find out rowsize
	long SliceSize() const { return sliceSize; }												//	accessor to find out slicesize
	long NElements() const { return nElements; }												//	accessor for nElements
	
	void ComputeIndex(ElementType *element, long &X, long &Y, long &Z) const						//	computes position & returns it in X, Y, Z
		{ // ComputeIndex()	
