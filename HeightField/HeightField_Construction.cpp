...
HeightField::HeightField(const My3dArrayf* vol)										//	constructor
	{ // HeightField()
...
//				to do this, I splice an additional component on to the top end of the existing component: thus, the existing component is valid for future tests
			...
			if (sComp->nextHi == sComp)									//	this only happens at plus infinity
				...
				} // else sComp isn't na hai highest arc
...
//	LoadFromNifti
void HeightField::LoadVolume(const My3dArrayf* vol)
{ // LoadFromNifti()
	if (vol == 0) { printf("Ivalid Volume.\n"); throw 0; }
	...
	zDim = vol->GetDimSize(2);
	yDim = vol->GetDimSize(1);
	xDim = vol->GetDimSize(0);											//	read in the dimensions
	...
	{ // E i
		...
		for (j = 0; j < yDim; j++)
			for (k = 0; k < zDim; k++)
			{ // E ijk
				float nextHeight = vol->At(MyVec3i(i,j,k));								//	temporary variable for reading in from array
				height(i, j, k) = nextHeight;									//	store it in the array
				sampleSum += nextHeight;						//	add to the running sum
				*heightWalk = &(height(i, j, k));								//	copy a pointer into sorting array
				if (nextHeight > maxHeight) maxHeight = nextHeight;				//	update maxHeight
